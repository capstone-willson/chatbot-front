...
class ChatWindowFooter extends HTMLElement {	
	...
	sendAndReply() {
		...
		if (this.canRemoteChat) {
			...
			xhr.open(`POST`, `https://hanyang-chatbot.kro.kr:8000/v1/db/questions/add`)
	...
	replyChat(text) {
		...
		xhr.open(`POST`, `https://hanyang-chatbot.kro.kr:8000/v1/chat`)
		xhr.addEventListener(`readystatechange`, () => {
			if (xhr.readyState === xhr.DONE) {
				if (xhr.status === 200 || xhr.status === 201) {	
	...
	chatQuestionAnalysis(text) {
		...
		if (checkbox.checked) {
			...
			xhr.open(`POST`, `https://hanyang-chatbot.kro.kr:8000/v1/analysis/similarity/morphs`)
			xhr.addEventListener(`readystatechange`, () => {
				if (xhr.readyState === xhr.DONE) {
					if (xhr.status === 200 || xhr.status === 201) {		
						console.info(JSON.parse(xhr.responseText))
	...
	insertContent(json) {
		...
		return {
			...
			writeText() {
				chatBody.reply(`
					<form class='input-form' action='https://hanyang-chatbot.kro.kr:8000/v1/db/questions/add' method='post' target='ifra'>
	...
	replyByPingpongAPI(text) {
		...
		xhr.open(`GET`, `https://hanyang-chatbot.kro.kr:8080/https://pingpong.us/api/reaction.php?custom=basic&query=${encodeURIComponent(text)}`)	
		xhr.setRequestHeader(`x-requested-with`, `XMLHttpRequest`)
		xhr.addEventListener(`readystatechange`, () => {
			if(xhr.readyState === COMPLETED) {
				if(xhr.status === OK) {
